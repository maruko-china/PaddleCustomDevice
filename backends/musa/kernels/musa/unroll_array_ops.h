...
struct UnrollFillConstant {
  template <typename T>
  __host__ __device__ inline static void Run(T *data, T val) {
    data[kStart] = val;
...
struct UnrollFillConstant<kStart, kEnd, true> {
  template <typename T>
  __host__ __device__ inline static void Run(T *data UNUSED, T val UNUSED) {}
};

template <size_t kStart, size_t kEnd, bool kStop>
struct UnrollAssign {
  template <typename Tin, typename Tout>
  __host__ __device__ inline static void Run(const Tin *d1, Tout *d2) {
...
struct UnrollAssign<kStart, kEnd, true> {
  template <typename Tin, typename Tout>
  __host__ __device__ inline static void Run(const Tin *d1 UNUSED, Tout *d2 UNUSED) {}
};
...
struct UnrollCompare<kStart, kEnd, true> {
  template <typename T>
  __host__ __device__ inline constexpr static bool Run(const T *d1 UNUSED,
                                              const T *d2 UNUSED) {
    return true;
  }
};
...
struct UnrollProduct {
  template <typename T>
  __host__ __device__ inline static T Run(const T *d) {
    return d[kStart] *
...
struct UnrollProduct<kStart, kEnd, true> {
  template <typename T>
  __host__ __device__ inline constexpr static T Run(const T *d UNUSED) {
    return 1;
  }
};
}  // namespace musa_detail
