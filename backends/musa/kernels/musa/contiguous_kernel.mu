...
__global__ void ContiguousCaseZeroFunc(
    const T* input_data,
    T* out_data,
    musa::Array<int64_t, phi::DDim::kMaxRank + 1> input_stride) {
...
__global__ void ContiguousDefaultFunc(
    const T* input_data,
    musa::Array<int64_t, phi::DDim::kMaxRank + 1> input_stride,
    musa::Array<int64_t, phi::DDim::kMaxRank + 1> dims,
    const int64_t numel,
    T* out_data) {
...
bool LaunchContiguousCazeZeroKernel(
    ...
    const T* input_data,
    const musa::Array<int64_t, phi::DDim::kMaxRank + 1>& input_stride,
    const musa::Array<int64_t, phi::DDim::kMaxRank + 1>& input_dims,
    int rank,
    T* output_data) {
  if (rank > 6) {
    return false;
...
bool LaunchContiguousCazeOneKernel(
    ...
    T* output_data) {
  dim3 grid(1, 1, 1), block(1, 1, 1);
  musa::Array<int64_t, 6> cur_input_dims;
  block.x = 512;
...
void ContiguousKernel(const Context& dev_ctx,
  ...
  musa::Array<int64_t, phi::DDim::kMaxRank + 1> input_stride;
  musa::Array<int64_t, phi::DDim::kMaxRank + 1> input_dims;
